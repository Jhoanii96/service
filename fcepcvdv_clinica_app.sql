...
-- Tiempo de generación: 01-09-2020 a las 05:17:12
...
-- Procedimientos
--
CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `actualizar_perfil` (IN `idUser` INT, IN `idDoctor` INT, IN `nombre` VARCHAR(80) CHARSET utf8, IN `apellidopa` VARCHAR(50) CHARSET utf8, IN `apellidoma` VARCHAR(50) CHARSET utf8, IN `especialidad` INT, IN `dni` VARCHAR(8) CHARSET utf8, IN `cmp` VARCHAR(6) CHARSET utf8, IN `pais` INT, IN `departamento` INT, IN `provincia` INT, IN `distrito` INT, IN `telefono1` VARCHAR(20) CHARSET utf8, IN `telefono2` VARCHAR(20) CHARSET utf8, IN `celular1` VARCHAR(20) CHARSET utf8, IN `celular2` VARCHAR(20) CHARSET utf8, IN `precioconsulta` FLOAT, IN `diapago` DATE, IN `dont_edit_photo` VARCHAR(1) CHARSET utf8, IN `image` VARCHAR(250) CHARSET utf8)  BEGIN
...
CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `getQuestionnaire` (IN `idUser` INT)  SELECT de.Id_Detalle_Cuestionario AS Id_Detalle,de.Pregunta FROM cuestionario cu INNER JOIN detalle_cuestionario de ON
de.Id_Cuestionario = cu.Id_Cuestionario WHERE 
cu.Id_Usuario = idUser AND de.Mostrar = 1$$

CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `insertar_cita` (IN `usersearch` INT, IN `datecita` DATE, IN `timecita` VARCHAR(10) CHARSET utf8, IN `username` VARCHAR(60) CHARSET utf8)  BEGIN
...
CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `insertar_cita_consulta` (IN `datecita` VARCHAR(15) CHARSET utf8, IN `timecita` VARCHAR(15) CHARSET utf8, IN `dnipaciente` VARCHAR(15) CHARSET utf8, IN `username` VARCHAR(60) CHARSET utf8)  BEGIN

	select @idpaciente := Id_Paciente from paciente where Documento = dnipaciente;
    select @idusuario := Id_Usuario from usuario where Nombre = username;
...
CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `insertar_codigo` (IN `codigo` VARCHAR(6) CHARSET utf8)  BEGIN
...
CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `insertar_paciente_cita` (IN `dni` VARCHAR(30) CHARSET utf8, IN `nombre` VARCHAR(60) CHARSET utf8, IN `apellidopa` VARCHAR(60) CHARSET utf8, IN `apellidoma` VARCHAR(60) CHARSET utf8, IN `genero` VARCHAR(30) CHARSET utf8, IN `celular` VARCHAR(30) CHARSET utf8, IN `fechana` DATE, IN `correo` VARCHAR(120) CHARSET utf8, IN `procedencia` VARCHAR(200) CHARSET utf8, IN `username` VARCHAR(60) CHARSET utf8)  BEGIN
...
CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `insertar_registro` (IN `especialidad` INT, IN `pais` INT, IN `departamento` INT, IN `provincia` INT, IN `distrito` INT, IN `cmp` VARCHAR(10) CHARSET utf8, IN `dni` VARCHAR(10) CHARSET utf8, IN `nombre` VARCHAR(80) CHARSET utf8, IN `apellidop` VARCHAR(50) CHARSET utf8, IN `apellidom` VARCHAR(50) CHARSET utf8, IN `address1` VARCHAR(200) CHARSET utf8, IN `gen` VARCHAR(1) CHARSET utf8, IN `cellphone` VARCHAR(20) CHARSET utf8, IN `fn` DATE, IN `price` DECIMAL(18,2), IN `username` VARCHAR(300) CHARSET utf8, IN `new_password` VARCHAR(50) CHARSET utf8, IN `dconsulta` VARCHAR(200) CHARSET utf8, IN `email` VARCHAR(200) CHARSET utf8, IN `isactive` TINYINT, IN `fecha_activacion` DATE, IN `usersearch` INT)  BEGIN
...
CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `mostrar_citas_consulta` (IN `fecha` VARCHAR(20) CHARSET utf8, IN `user_name` VARCHAR(60) CHARSET utf8)  BEGIN
...
CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `mostrar_detalle_cita` (IN `codcita` INT, IN `user_name` VARCHAR(60) CHARSET utf8)  BEGIN
...
CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `mostrar_detalle_historial` (IN `codhistorial` INT, IN `user_name` VARCHAR(60) CHARSET utf8)  BEGIN

	select * from v_detalle_historia vd where vd.id = codhistorial and vd.username = user_name; 

END$$

CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `mostrar_lista_citas` (IN `nombresearch` VARCHAR(60) CHARSET utf8, IN `filter` VARCHAR(1) CHARSET utf8, IN `user_name` VARCHAR(60) CHARSET utf8)  BEGIN
...
CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `mostrar_perfil` (IN `user_name` VARCHAR(60) CHARSET utf8)  BEGIN

	select * from v_perfil vp where vp.username = user_name; 

END$$

CREATE DEFINER=`cpses_fcaigq65yo`@`localhost` PROCEDURE `showProfile` (IN `name` VARCHAR(30) CHARSET utf8)  SELECT 
	us.Id_Usuario, 
    doc.Id_Doctor, 
    doc.Nombres, 
...
INSERT INTO `archivo` (`Id_Archivo`, `Nombre`, `Subnombre`, `Enlace`, `Size`, `FechaCreado`, `FechaModificado`, `Borrado`, `CreadoPor`, `ModificadoPor`, `Id_Historia_Clinica`, `Id_Tipo_Archivo`) VALUES
(111, 'Nombre 1', 'Nombre 1', 'src/assets/media/images/historia_clinica/0826200612181598458338.5551.jpeg', 85167, '2020-08-26 11:12:18', '0000-00-00 00:00:00', 0, 1, NULL, 84, '1'),
(112, 'Nombre 2', 'Nombre 2', 'src/assets/media/images/historia_clinica/0826200618051598458685.9533.jpeg', 979433, '2020-08-26 11:18:05', '0000-00-00 00:00:00', 0, 1, NULL, 85, '1'),
(113, 'Nombre 3', 'Nombre 3', 'src/Documentos/0826200618051598458685.9533.vnd.openxmlformats-officedocument.wordprocessingml.document', 657806, '2020-08-26 11:18:05', '0000-00-00 00:00:00', 0, 1, NULL, 85, '4'),
(114, 'Nombre 4', 'Nombre 4', 'src/Documentos/0826200618051598458685.9534.pdf', 87176, '2020-08-26 11:18:05', '0000-00-00 00:00:00', 0, 1, NULL, 85, '3'),
(115, 'dante-dmc.jpg', 'dante-dmc.jpg', 'src/assets/media/images/historia_clinica/dante-dmc.jpg', 651429, '2020-08-31 18:24:02', NULL, 0, 1, NULL, 86, '1'),
(116, 'DMC-Dante.jpg', 'DMC-Dante.jpg', 'src/assets/media/images/historia_clinica/DMC-Dante.jpg', 14098, '2020-08-31 18:24:02', NULL, 0, 1, NULL, 86, '1'),
(117, '0901200336541598924214.6335.jpeg', '0901200336541598924214.6335.jpeg', 'src/assets/media/images/historia_clinica/0901200336541598924214.6335.jpeg', 59538, '2020-08-31 20:36:54', NULL, 0, 1, NULL, 94, '1'),
(118, '0901200336541598924214.6335.jpeg', '0901200336541598924214.6335.jpeg', 'src/assets/media/images/historia_clinica/0901200336541598924214.6335.jpeg', 134122, '2020-08-31 20:36:54', NULL, 0, 1, NULL, 94, '1'),
(119, '0901200336541598924214.6336.jpeg', '0901200336541598924214.6336.jpeg', 'src/assets/media/images/historia_clinica/0901200336541598924214.6336.jpeg', 14098, '2020-08-31 20:36:54', NULL, 0, 1, NULL, 94, '1'),
(120, '0901200338351598924315.7157.pdf', '0901200338351598924315.7157.pdf', 'src/Documentos/0901200338351598924315.7157.pdf', 2297010, '2020-08-31 20:38:35', NULL, 0, 1, NULL, 86, '3'),
(121, '0901200338351598924315.7158.pdf', '0901200338351598924315.7158.pdf', 'src/Documentos/0901200338351598924315.7158.pdf', 2179090, '2020-08-31 20:38:35', NULL, 0, 1, NULL, 86, '3'),
(122, '0901200346211598924781.1582.pdf', '0901200346211598924781.1582.pdf', 'src/Documentos/0901200346211598924781.1582.pdf', 2297010, '2020-08-31 20:46:21', NULL, 0, 1, NULL, 96, '3'),
(123, '0901200346211598924781.1583.pdf', '0901200346211598924781.1583.pdf', 'src/Documentos/0901200346211598924781.1583.pdf', 11661100, '2020-08-31 20:46:21', NULL, 0, 1, NULL, 96, '3'),
(124, '0901200346211598924781.1584.pdf', '0901200346211598924781.1584.pdf', 'src/Documentos/0901200346211598924781.1584.pdf', 2179090, '2020-08-31 20:46:21', NULL, 0, 1, NULL, 96, '3'),
(125, '0901200349061598924946.2905.pdf', '0901200349061598924946.2905.pdf', 'src/Documentos/0901200349061598924946.2905.pdf', 2297010, '2020-08-31 20:49:06', NULL, 0, 1, NULL, 97, '3'),
(126, '0901200349061598924946.2906.pdf', '0901200349061598924946.2906.pdf', 'src/Documentos/0901200349061598924946.2906.pdf', 11661100, '2020-08-31 20:49:06', NULL, 0, 1, NULL, 97, '3'),
(127, '0901200349061598924946.2906.pdf', '0901200349061598924946.2906.pdf', 'src/Documentos/0901200349061598924946.2906.pdf', 2179090, '2020-08-31 20:49:06', NULL, 0, 1, NULL, 97, '3'),
(128, '0901200358201598925500.5482.png', '0901200358201598925500.5482.png', 'src/assets/media/images/historia_clinica/0901200358201598925500.5482.png', 134620, '2020-08-31 20:58:20', NULL, 0, 1, NULL, 98, '2'),
(129, '0901200358201598925500.5482.png', '0901200358201598925500.5482.png', 'src/assets/media/images/historia_clinica/0901200358201598925500.5482.png', 110020, '2020-08-31 20:58:20', NULL, 0, 1, NULL, 98, '2'),
(130, '0901200358201598925500.5483.pdf', '0901200358201598925500.5483.pdf', 'src/Documentos/0901200358201598925500.5483.pdf', 2297010, '2020-08-31 20:58:20', NULL, 0, 1, NULL, 98, '3'),
(131, 'Mayra.jpg', 'Mayra.jpg', 'src/assets/media/images/historia_clinica/Mayra.jpg', 59538, '2020-08-31 21:05:56', NULL, 0, 1, NULL, 99, '1'),
(132, 'dante-dmc.jpg', 'dante-dmc.jpg', 'src/assets/media/images/historia_clinica/dante-dmc.jpg', 651429, '2020-08-31 21:05:56', NULL, 0, 1, NULL, 99, '1'),
(133, 'DMC-Dante.jpg', 'DMC-Dante.jpg', 'src/assets/media/images/historia_clinica/DMC-Dante.jpg', 14098, '2020-08-31 21:05:56', NULL, 0, 1, NULL, 99, '1'),
(134, 'doom.jpg', 'doom.jpg', 'src/assets/media/images/historia_clinica/doom.jpg', 134122, '2020-08-31 21:08:50', NULL, 0, 1, NULL, 100, '1'),
(135, 'dante-dmc.jpg', 'dante-dmc.jpg', 'src/assets/media/images/historia_clinica/dante-dmc.jpg', 651429, '2020-08-31 21:08:50', NULL, 0, 1, NULL, 100, '1'),
(136, 'DMC-Dante.jpg', 'DMC-Dante.jpg', 'src/assets/media/images/historia_clinica/DMC-Dante.jpg', 14098, '2020-08-31 21:08:50', NULL, 0, 1, NULL, 100, '1'),
(137, 'kaguya.jpg', 'kaguya.jpg', 'src/assets/media/images/historia_clinica/kaguya.jpg', 78237, '2020-08-31 21:08:50', NULL, 0, 1, NULL, 100, '1'),
(138, 'Redes CISCO. Curso práctico de formación para la certificación CCNA_compressed.pdf', 'Redes CISCO. Curso práctico de formación para la certificación CCNA_compressed.pdf', 'src/Documentos/Redes CISCO. Curso práctico de formación para la certificación CCNA_compressed.pdf', 11661100, '2020-08-31 21:08:50', NULL, 0, 1, NULL, 100, '3'),
(139, 'CodeIgniter_3_1_9_Manual_Esp.pdf', 'CodeIgniter_3_1_9_Manual_Esp.pdf', 'src/Documentos/CodeIgniter_3_1_9_Manual_Esp.pdf', 2179090, '2020-08-31 21:08:50', NULL, 0, 1, NULL, 100, '3'),
(140, 'Mayra.jpg', 'Mayra.jpg', 'src/assets/media/images/historia_clinica/Mayra.jpg', 59538, '2020-08-31 21:15:08', NULL, 0, 1, NULL, 101, '1'),
(141, 'doom.jpg', 'doom.jpg', 'src/assets/media/images/historia_clinica/doom.jpg', 134122, '2020-08-31 21:15:08', NULL, 0, 1, NULL, 101, '1'),
(142, 'Redes CISCO. Curso práctico de formación para la certificación CCNA_compressed.pdf', 'Redes CISCO. Curso práctico de formación para la certificación CCNA_compressed.pdf', 'src/Documentos/Redes CISCO. Curso práctico de formación para la certificación CCNA_compressed.pdf', 11661100, '2020-08-31 21:15:08', NULL, 0, 1, NULL, 101, '3'),
(143, 'CodeIgniter_3_1_9_Manual_Esp.pdf', 'CodeIgniter_3_1_9_Manual_Esp.pdf', 'src/Documentos/CodeIgniter_3_1_9_Manual_Esp.pdf', 2179090, '2020-08-31 21:15:08', NULL, 0, 1, NULL, 101, '3');
...
(20, 6, 25, '2020-08-22 22:22:52', '2020-08-23 20:30:00', 50, 0, 0, 0, 0),
(21, 6, 13, '2020-08-23 01:44:04', '2020-08-23 01:00:00', 50, 0, 0, 0, 0),
(22, 2, 10, '2020-08-25 00:57:33', '2020-08-25 19:30:00', 50, 0, 0, 0, 0),
(23, 1, 1, '2020-09-01 09:01:05', '2020-09-01 17:12:00', 50, 0, 0, 0, 0);
...
INSERT INTO `detalle_cuestionario` (`Id_Detalle_Cuestionario`, `Id_Cuestionario`, `Pregunta`, `Mostrar`) VALUES
(22, 24, 'tabaco', 0),
(95, 24, '123456', 1),
(96, 24, '4548484', 0),
(97, 24, 'pregunta 1', 1),
(116, 26, 'SIDA', 1),
(117, 24, 'TIENE TETANO', 1),
(118, 24, 'FUMA', 0),
(119, 24, 'PREGUNTA 3', 0),
(120, 27, 'PREGUNTA 1', 1),
(121, 27, 'PREGUNTA 2', 1),
...
(133, 97, 'CLARO PRRO', 1, 1),
(134, 117, 'ALBERTH TIENE XD', 1, 1),
(137, 95, '12345', 1, 1),
(138, 95, 'si', 28, 1),
(139, 97, 'no', 28, 1),
(140, 117, 'si', 28, 1),
...
(76, 2, 9, 'HUAYLLACAYAN', '020510'),
(77, 2, 9, 'LA PRIMAVERA', '020511'),
...
(89, 3, 10, 'SAN PEDRO DE CACHORA', '030108'),
...
(102, 3, 11, 'SAN ANTONIO DE CACHI', '030212'),
(103, 3, 11, 'SANJERÓNIMO', '010518'),
(104, 3, 11, 'SAN MIGUEL DE CHACCRAMPA', '030214'),
(105, 3, 11, 'SANTA MARIA DE CHICMO', '030215'),
...
(113, 3, 12, 'JUAN ESPINOZA MEDRANO', '030304'),
...
(124, 3, 13, 'JUSTO APU SAHUARAURA', '030408'),
(125, 3, 13, 'LUCRE', '030409'),
(126, 3, 13, 'POCOHUANCA', '030410'),
(127, 3, 13, 'SAN JUAN DE CHACÑA', '030411'),
(128, 3, 13, 'SAÑAYCA', '030412'),
...
(153, 3, 16, 'MICAELA BASTIDAS', '030706'),
(154, 3, 16, 'PATAYPAMPA', '030707'),
(155, 3, 16, 'PROGRESO', '030708'),
(156, 3, 16, 'SAN ANTONIO', '030709'),
(157, 3, 16, 'SANTA ROSA', '010610'),
...
(164, 4, 17, 'ALTO SELVA ALEGRE', '040102'),
(165, 4, 17, 'CAYMA', '040103'),
(166, 4, 17, 'CERRO COLORADO', '040104'),
(167, 4, 17, 'CHARACATO', '040105'),
(168, 4, 17, 'CHIGUATA', '040106'),
(169, 4, 17, 'JACOBO HUNTER', '040107'),
(170, 4, 17, 'LAJOYA', '040108'),
(171, 4, 17, 'MARIANO MELGAR', '040109'),
...
(177, 4, 17, 'QUEQUEÑA', '040115'),
(178, 4, 17, 'SABANDIA', '040116'),
(179, 4, 17, 'SACHACA', '040117'),
(180, 4, 17, 'SAN JUAN DE SIGUAS', '040118'),
(181, 4, 17, 'SAN JUAN DE TARUCANI', '040119'),
(182, 4, 17, 'SANTA ISABEL DE SIGUAS', '040120'),
(183, 4, 17, 'SANTA RITA DE SIGUAS', '040121'),
...
(191, 4, 17, 'JOSE LUIS BUSTAMANTE Y RIVERO', '040129'),
(192, 4, 18, 'CAMANA', '040201'),
(193, 4, 18, 'JOSE MARIA QUIMPER', '040202'),
(194, 4, 18, 'MARIANO NICOLÁS VALCÁRCEL', '040203'),
(195, 4, 18, 'MARISCALC?CERES', '040204'),
(196, 4, 18, 'NICOLÁS DE PIÉROLA', '040205'),
(197, 4, 18, 'OCOÑA', '040206'),
(198, 4, 18, 'QUILCA', '040207'),
(199, 4, 18, 'SAMUEL PASTOR', '040208'),
(200, 4, 19, 'CARAVEL', '040301'),
(201, 4, 19, 'ACARI', '040302'),
(202, 4, 19, 'ATICO', '040303'),
(203, 4, 19, 'ATIQUIPA', '040304'),
(204, 4, 19, 'BELLA UNIÓN', '040305'),
...
(240, 4, 21, 'SAN ANTONIO DE CHUCA', '040514'),
...
(280, 5, 25, 'SAN JOSÉ DE TICLLAS', '050109'),
(281, 5, 25, 'SAN JUAN BAUTISTA', '050110'),
(282, 5, 25, 'SANTIAGO DE PISCHA', '050111'),
...
(317, 5, 30, 'CARMEN SALCEDO', '050604'),
(318, 5, 30, 'CHAVIÑA', '050605'),
...
(328, 5, 30, 'SAN CRISTÓBAL', '010516'),
(329, 5, 30, 'SANJUAN', '021909'),
(330, 5, 30, 'SANPEDRO', '021409'),
(331, 5, 30, 'SAN PEDRO DE PALCO', '050618'),
(332, 5, 30, 'SANCOS', '050301'),
(333, 5, 30, 'SANTA ANA DE HUAYCAHUACHO', '050620'),
(334, 5, 30, 'SANTALUCÍA', '050621'),
(335, 5, 31, 'CORACORA', '050701'),
(336, 5, 31, 'CHUMPI', '050702'),
(337, 5, 31, 'CORONEL CASTAÑEDA', '050703'),
(338, 5, 31, 'PACAPAUSA', '050704'),
(339, 5, 31, 'PULLO', '050705'),
(340, 5, 31, 'PUYUSCA', '050706'),
(341, 5, 31, 'SAN FRANCISCO DE RAVACAYCO', '050707'),
...
(350, 5, 32, 'SAN JAVIER DE ALPABAMBA', '050808'),
(351, 5, 32, 'SAN JOSÉ DE USHUA', '050809'),
(352, 5, 32, 'SARASARA', '050810'),
(353, 5, 33, 'QUEROBAMBA', '050901'),
(354, 5, 33, 'BELEN CHALCOS', '050902'),
(355, 5, 33, 'CHILCAYOC', '050904'),
(356, 5, 33, 'HUACAÑA', '050905'),
(357, 5, 33, 'MORCOLLA', '050906'),
(358, 5, 33, 'PAICO', '050907'),
(359, 5, 33, 'SAN PEDRO DE LARCAY', '050908'),
(360, 5, 33, 'SAN SALVADOR DE QUIJE', '050909'),
(361, 5, 33, 'SANTIAGO DE PAUCARAY', '050910'),
...
(375, 5, 35, 'VILCASHUAMÁN', '051101'),
(376, 5, 35, 'ACCOMARCA', '051102'),
(377, 5, 35, 'CARHUANCA', '051103'),
(378, 5, 35, 'CONCEPCIÓN', '051104'),
...
(384, 6, 36, 'ASUNCIÓN', '010102'),
(385, 6, 36, 'CHETILLA', '060103'),
(386, 6, 36, 'COSPÁN', '060104'),
(387, 6, 36, 'ENCAÑADA', '060105'),
(388, 6, 36, 'JES?S', '060106'),
(389, 6, 36, 'LLACANORA', '060107'),
(390, 6, 36, 'LOS BAÑOS DELINCA', '060108'),
...
(399, 6, 38, 'CELENDÍN', '060301'),
(400, 6, 38, 'CHUMUCH', '060302'),
(401, 6, 38, 'CORTEGANA', '060303'),
(402, 6, 38, 'HUASMÍN', '060304'),
(403, 6, 38, 'JORGE CHÁVEZ', '060305'),
(404, 6, 38, 'JOSÉ GÁLVEZ', '060306'),
(405, 6, 38, 'MIGUEL IGLESIAS', '060307'),
(406, 6, 38, 'OXAMARCA', '060308'),
(407, 6, 38, 'SOROCHUCO', '060309'),
(408, 6, 38, 'SUCRE', '060310'),
(409, 6, 38, 'UTCO', '060311'),
(410, 6, 38, 'LA LIBERTAD DE PALLÁN', '060312'),
...
(445, 6, 41, 'SAN ANDRÉS CUTERVO', '060608'),
(446, 6, 41, 'SAN JUAN DE CUTERVO', '060609'),
(447, 6, 41, 'SAN LUIS DE LUCMA', '060610'),
(448, 6, 41, 'SANTACRUZ', '021208'),
(449, 6, 41, 'SANTO DOMINGO LA CAPILLA', '060612'),
(450, 6, 41, 'SANTO TOMÁS', '010521'),
(451, 6, 41, 'SOCOTA', '060614'),
(452, 6, 41, 'TORIBIO CASANOVA', '060615'),
(453, 6, 42, 'BAMBAMARCA', '060701'),
(454, 6, 42, 'CHUGUR', '060702'),
(455, 6, 42, 'HUALGAYOC', '060703'),
(456, 6, 43, 'JAÉN', '060801'),
...
(466, 6, 43, 'SAN JOSE DEL ALTO', '060811'),
...
(473, 6, 44, 'SAN JOSE DE LOURDES', '060906'),
(474, 6, 44, 'TABACONAS', '060907'),
(475, 6, 45, 'PEDRO GALVEZ', '061001'),
(476, 6, 45, 'CHANCAY', '061002'),
(477, 6, 45, 'EDUARDO VILLANUEVA', '061003'),
(478, 6, 45, 'GREGORIOPITA', '061004'),
(479, 6, 45, 'ICHOCÁN', '061005'),
(480, 6, 45, 'JOSE MANUEL QUIROZ', '061006'),
(481, 6, 45, 'JOSE SABOGAL', '061007'),
(482, 6, 46, 'SANMIGUEL', '050501'),
(483, 6, 46, 'BOLÍVAR CALQUIS', 'NULL'),
...
(651, 8, 64, 'HUAYLLAY GRANDE', '090308'),
(652, 8, 64, 'JULCAMARCA', '090309'),
(653, 8, 64, 'SAN ANTONIO DE ANTAPARCO', '090310'),
(654, 8, 64, 'SANTO TOMÁS DE PATA', '090311'),
...
(676, 8, 66, 'SAN MIGUEL DE MAYOCC', '090508'),
(677, 8, 66, 'SAN PEDRO DE CORIS', '090509'),
(678, 8, 66, 'PACHAMARCA', '090510'),
(679, 8, 66, 'COSME', 'NULL'),
(680, 8, 67, 'HUAYTARÁ', '090601'),
(681, 8, 67, 'AYAVI', '090602'),
(682, 8, 67, 'CÓRDOVA', '090603'),
(683, 8, 67, 'HUAYACUNDO ARMA', '090604'),
(684, 8, 67, 'LARAMARCA', '090605'),
(685, 8, 67, 'OCOYO', '090606'),
(686, 8, 67, 'PILPICHACA', '090607'),
(687, 8, 67, 'QUERCO', '090608'),
(688, 8, 67, 'QUITO-ARMA', '090609'),
(689, 8, 67, 'SAN ANTONIO DE CUSICANCHA', '090610'),
(690, 8, 67, 'SAN FRANCISCO DE SANGAYAICO', '090611'),
(691, 8, 67, 'SANISIDRO', '090612'),
(692, 8, 67, 'SANTIAGO DE CHOCORVOS', '090613'),
(693, 8, 67, 'SANTIAGO DE QUIRAHUARA', '090614'),
(694, 8, 67, 'SANTO DOMINGO DE CAPILLAS', '090615'),
...
(747, 9, 73, 'CHAVIN DE PARIARCA', '100503'),
(748, 9, 73, 'JACAS GRANDE', '100504'),
(749, 9, 73, 'JIRCAN', '100505'),
(750, 9, 73, 'MIRAFLORES', '040110'),
(751, 9, 73, 'MONZÓN', '100507'),
(752, 9, 73, 'PUNCHAO', '100508'),
(753, 9, 73, 'PUÑOS', '100509'),
(754, 9, 73, 'SINGA', '100510'),
(755, 9, 73, 'TANTAMAYO', '100511'),
(756, 9, 74, 'RUPA-RUPA', '100601'),
(757, 9, 74, 'DANIEL ALOMIA ROBLES', '100602'),
(758, 9, 74, 'HERMILIO VALDIZÁN', '100603'),
(759, 9, 74, 'JOSÉ CRESPO Y CASTILLO', '100604'),
(760, 9, 74, 'LUYANDO', '100605'),
(761, 9, 74, 'MARIANO DAMASO BERAÚN', '100606'),
(762, 9, 75, 'HUACRACHUCO', '100701'),
(763, 9, 75, 'CHOLÓN', '100702'),
...
(849, 11, 85, 'PUCARÁ', '060808'),
(850, 11, 85, 'QUICHUAY', '120127'),
(851, 11, 85, 'QUILCAS', '120128'),
(852, 11, 85, 'SANAGUSTÍN', '120129'),
(853, 11, 85, 'SAN JERÓNIMO DE TUNAN', '120130'),
(854, 11, 85, 'SA?O', '120132'),
(855, 11, 85, 'SAPALLANGA', '120133'),
(856, 11, 85, 'SICAYA', '120134'),
(857, 11, 85, 'SANTO DOMINGO DE ACOBAMBA', '120135'),
(858, 11, 85, 'VIQUES', '120136'),
(859, 11, 86, 'CONCEPCIÓN', '051104'),
(860, 11, 86, 'ACO', '020902'),
(861, 11, 86, 'ANDAMARCA', '120203'),
(862, 11, 86, 'CHAMBARÁ', '120204'),
(863, 11, 86, 'COCHAS', '021405'),
(864, 11, 86, 'COMAS', '120206'),
(865, 11, 86, 'HEROINAS TOLEDO', '120207'),
(866, 11, 86, 'MANZANARES', '120208'),
(867, 11, 86, 'MARISCAL CASTILLA', '010113'),
(868, 11, 86, 'MATAHUASI', '120210'),
(869, 11, 86, 'MITO', '120211'),
(870, 11, 86, 'NUEVE DE JULIO', '120212'),
(871, 11, 86, 'ORCOTUNA', '120213'),
(872, 11, 86, 'SAN JOSE DE QUERO', '120214'),
(873, 11, 86, 'SANTA ROSA DE OCOPA', '120215'),
(874, 11, 87, 'CHANCHAMAYO', '120301'),
(875, 11, 87, 'PEREN', '120302'),
(876, 11, 87, 'PICHANAQUI', '120303'),
(877, 11, 87, 'SAN LUIS DE SHUARO', '120304'),
(878, 11, 87, 'SAN RAMÓN', '120305'),
...
(891, 11, 88, 'JULCÁN', '120412'),
(892, 11, 88, 'LEONOR ORDOÑEZ', '120413'),
...
(91, 1, 1, NULL, '2020-08-28 19:57:46', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'asdasda'),
(92, 1, 1, NULL, '2020-08-31 18:10:26', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'asdasd'),
(93, 1, 1, NULL, '2020-08-31 18:24:02', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'asdasd'),
(94, 1, 1, NULL, '2020-08-31 20:36:54', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'asdasdasd'),
(95, 1, 1, NULL, '2020-08-31 20:38:35', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'sadsad'),
(96, 1, 1, NULL, '2020-08-31 20:46:21', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'alskdjla'),
(97, 28, 1, NULL, '2020-08-31 20:49:06', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'asdkjalsjlkajdslk'),
(98, 14, 1, NULL, '2020-08-31 20:58:20', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'alskdjlkj'),
(99, 12, 1, NULL, '2020-08-31 21:05:56', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'asdasd'),
(100, 14, 1, NULL, '2020-08-31 21:08:50', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'iuhiuhiuh'),
(101, 11, 1, NULL, '2020-08-31 21:15:08', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'lasjdlkasjdlkj'),
(102, 1, 1, NULL, '2020-08-31 21:16:56', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'asjdkajsd'),
(103, 1, 1, NULL, '2020-08-31 21:18:25', 'te:', 'resultado', '1\r\n2\r\n3\r\n', 'diag', 'laksjdlkasd'),
(104, 10, 2, 22, '2020-08-31 21:23:31', 'kasdhkjah\r\nalsjdalk', 'saluuute', 'alsjdlkasjdlk\r\nslkjdlaksdj', 'asldajsldk', '654654654');
...
(20, '70143455', 'JOSUE', 'ALDAIR', 'MAMANI', 'M', '1997-08-27', '', 'jsue@hotmail.com', 'PERU', '', 'ESTUDIANTE', 6),
...
(25, '44444444', 'YIMI', 'MUÑOZ', 'MUÑOZ', 'M', '1997-08-12', '959856137', 'ronaldo@hotmail.com', 'PERU-TACNA', '', 'DOCENTE', 6),
(26, '78787878', 'DALTHON ', 'MAMANI', 'MAMANI', 'M', '1995-05-21', '959856137', '', 'PERU-TACNA', '', '', 6),
(27, '46464646', 'EDWIN', 'HINOJOZA', 'RAMOS', 'M', '1997-02-16', '959856137', '', 'PERU-TACNA', '', '', 6),
(28, '48468468', 'CARLOS ASDDAS', 'HUAYNAPATA', 'KO', 'F', '2020-08-04', '646846546', '', 'PERUANA', 'ASD', 'ASD', 1);
...
(103, 13, 'FERREÑAFE', '1402'),
(104, 13, 'LAMBAYEQUE', '1403'),
(105, 14, 'MAYNAS', '1601'),
(106, 14, 'ALTO AMAZONAS', '1602'),
(107, 14, 'LORETO', '1603'),
(108, 14, 'MARISCAL RAMON CASTILLA', '1604'),
(109, 14, 'REQUENA', '1605'),
(110, 14, 'UCAYALI', '1606'),
(111, 14, 'DATEM DEL MARAÑON', '1607'),
(112, 15, 'TAMBOPATA', '1701'),
(113, 15, 'MANU', '1702'),
(114, 15, 'TAHUAMANU', '1703'),
(115, 16, 'MARISCAL NIETO', '1801'),
(116, 16, 'GENERAL SANCHEZ CERRO', '1802'),
(117, 16, 'ILO', '1803'),
(118, 17, 'PASCO', '1901'),
(119, 17, 'DANIEL ALCIDES CARRION', '1902'),
...
(133, 19, 'EL COLLAO', '2105'),
(134, 19, 'HUANCANE', '2106'),
(135, 19, 'LAMPA', '2107'),
(136, 19, 'MELGAR', '2108'),
(137, 19, 'MOHO', '2109'),
(138, 19, 'SAN ANTONIO DE PUTINA', '2110'),
(139, 19, 'SAN ROMAN', '2111'),
(140, 19, 'SANDIA', '2112'),
(141, 19, 'YUNGUYO', '2113'),
(142, 20, 'CORONEL PORTILLO', '2501'),
(143, 20, 'ATALAYA', '2502'),
(144, 20, 'PADRE ABAD', '2503'),
(145, 20, 'PURUS', '2504'),
(146, 21, 'MOYOBAMBA', '2201'),
(147, 21, 'BELLAVISTA', '2202'),
(148, 21, 'EL DORADO', '2203'),
(149, 21, 'HUALLAGA', '2204'),
(150, 21, 'LAMAS', '2205'),
(151, 21, 'MARISCAL CACERES', '2206'),
(152, 21, 'PICOTA', '2207'),
(153, 21, 'RIOJA', '2208'),
...
(168, 23, 'OYON', '1509'),
(169, 23, 'YAUYOS', '1510'),
(170, 25, 'PROV.CONST.DEL CALLAO', '0701'),
(171, 24, 'TUMBES', '2401'),
(172, 24, 'CONTRALMIRANTE VILLAR', '2402'),
...
--
-- Estructura Stand-in para la vista `v_detalle_cita`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_detalle_cita` (
`id` int(11)
,`nombre` varchar(30)
,`ape_pa` varchar(30)
,`ape_ma` varchar(30)
...
--
-- Estructura Stand-in para la vista `v_detalle_historia`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_detalle_historia` (
`id` int(11)
,`nombres` varchar(92)
,`dni` varchar(30)
,`genero` varchar(30)
...
,`edad` date
,`fecha_historia` varchar(19)
,`diag` text
,`ex_fi` text
,`anam` text
,`exam` text
,`fcreacion` varchar(19)
,`fcita` varchar(19)
,`estado` int(11)
,`atencion` int(11)
,`precio` int(11)
...
--
-- Estructura Stand-in para la vista `v_lista_archivo`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_lista_archivo` (
`id_archivo` int(11)
,`id_historial` int(11)
...
,`num_imagen` bigint(21)
,`num_archivo` bigint(21)
,`username` varchar(300)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`cpses_fcaigq65yo`@`localhost` SQL SECURITY DEFINER VIEW `v_departamento`  AS  select `de`.`Id_Pais` AS `idPais`,`de`.`Id_Departamento` AS `idDepartamento`,`de`.`Descripcion` AS `nombre_departamento` from `departamento` `de` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_detalle_cita`
--
...
--
-- Estructura para la vista `v_detalle_historia`
--
...
CREATE ALGORITHM=UNDEFINED DEFINER=`cpses_fcaigq65yo`@`localhost` SQL SECURITY DEFINER VIEW `v_distrito`  AS  select `di`.`Id_Provincia` AS `idProvincia`,`di`.`Id_Distrito` AS `idDistrito`,`di`.`Descripcion` AS `nombre_distrito` from `distrito` `di` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_lista_archivo`
--
DROP TABLE IF EXISTS `v_lista_archivo`;

CREATE ALGORITHM=UNDEFINED DEFINER=`cpses_fcaigq65yo`@`localhost` SQL SECURITY DEFINER VIEW `v_lista_archivo`  AS  select `ar`.`Id_Archivo` AS `id_archivo`,`hc`.`Id_historia_clinica` AS `id_historial`,`ar`.`Subnombre` AS `nombre_archivo`,`ar`.`Enlace` AS `url`,`ar`.`Borrado` AS `isdelete`,`ar`.`Size` AS `size`,`ar`.`FechaCreado` AS `fecha_creado`,`ta`.`Nombre` AS `tipo_nom`,`us`.`Nombre` AS `username` from (((`historia_clinica` `hc` join `usuario` `us` on(`us`.`Id_Usuario` = `hc`.`Id_Usuario`)) join `archivo` `ar` on(`ar`.`Id_Historia_Clinica` = `hc`.`Id_historia_clinica`)) join `tipo_archivo` `ta` on(`ta`.`Id_Tipo_Archivo` = `ar`.`Id_Tipo_Archivo`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`cpses_fcaigq65yo`@`localhost` SQL SECURITY DEFINER VIEW `v_lista_doctor`  AS  select `doc`.`Id_Doctor` AS `id`,concat(`doc`.`Nombres`,' ',`doc`.`Apellido_Paterno`,' ',`doc`.`Apellido_Materno`) AS `nombre` from (`doctor` `doc` join `usuario` `usu` on(`usu`.`Id_Doctor` = `doc`.`Id_Doctor`)) where `usu`.`Activo` = 1 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`cpses_fcaigq65yo`@`localhost` SQL SECURITY DEFINER VIEW `v_lista_historia_clinica`  AS  select `hc`.`Id_historia_clinica` AS `id_historia`,`pa`.`Id_Paciente` AS `id_paciente`,concat(`pa`.`Nombre`,' ',`pa`.`Apellido_Paterno`,' ',`pa`.`Apellido_Materno`) AS `nombre_paciente`,`pa`.`Fecha_Nacimiento` AS `fecha_nacimiento`,`hc`.`Fecha` AS `fecha_consulta`,(select count(`ars`.`Id_Archivo`) AS `numimg` from `archivo` `ars` where (`ars`.`Id_Tipo_Archivo` = 1 or `ars`.`Id_Tipo_Archivo` = 2) and `ars`.`Id_Historia_Clinica` = `hc`.`Id_historia_clinica`) AS `num_imagen`,(select count(`ars`.`Id_Archivo`) AS `numimg` from `archivo` `ars` where (`ars`.`Id_Tipo_Archivo` = 3 or `ars`.`Id_Tipo_Archivo` = 4) and `ars`.`Id_Historia_Clinica` = `hc`.`Id_historia_clinica`) AS `num_archivo`,`us`.`Nombre` AS `username` from ((`historia_clinica` `hc` join `paciente` `pa` on(`pa`.`Id_Paciente` = `hc`.`Id_Paciente`)) join `usuario` `us` on(`us`.`Id_Usuario` = `hc`.`Id_Usuario`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`cpses_fcaigq65yo`@`localhost` SQL SECURITY DEFINER VIEW `v_provincia`  AS  select `pr`.`Id_Departamento` AS `idDepartamento`,`pr`.`Id_Provincia` AS `idProvincia`,`pr`.`Descripcion` AS `nombre_pro` from `provincia` `pr` ;
...
--
-- Indices de la tabla `historia_clinica_predeterminado`
--
ALTER TABLE `historia_clinica_predeterminado`
  ADD PRIMARY KEY (`Id_Historia_Clinica_Predeterminado`);
...
--
ALTER TABLE `citas`
  MODIFY `Id_Citas` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=24;
...
--
ALTER TABLE `detalle_cuestionario_paciente`
  MODIFY `Id_Cuestionario_Paciente` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=154;
...
--
ALTER TABLE `historia_clinica`
  MODIFY `Id_historia_clinica` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=105;

--
-- AUTO_INCREMENT de la tabla `historia_clinica_predeterminado`
--
ALTER TABLE `historia_clinica_predeterminado`
  MODIFY `Id_Historia_Clinica_Predeterminado` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=37;
