...
CREATE DEFINER=`` PROCEDURE `getQuestionnaire` (IN `idUser` INT)  SELECT de.Id_Detalle_Cuestionario AS Id_Detalle,de.Pregunta FROM cuestionario cu INNER JOIN detalle_cuestionario de ON
de.Id_Cuestionario = cu.Id_Cuestionario WHERE 
cu.Id_Usuario = idUser$$

CREATE DEFINER=`` PROCEDURE `insertar_cita` (IN `usersearch` INT, IN `datecita` DATE, IN `timecita` VARCHAR(10), IN `username` VARCHAR(60))  BEGIN
...
CREATE DEFINER=`` PROCEDURE `insertar_codigo` (IN `codigo` VARCHAR(6))  BEGIN
...
CREATE DEFINER=`` PROCEDURE `insertar_paciente_cita` (IN `dni` VARCHAR(30), IN `nombre` VARCHAR(60), IN `apellidopa` VARCHAR(60), IN `apellidoma` VARCHAR(60), IN `genero` VARCHAR(30), IN `celular` VARCHAR(30), IN `fechana` DATE, IN `correo` VARCHAR(120), IN `procedencia` VARCHAR(200), IN `username` VARCHAR(60))  BEGIN
...
CREATE DEFINER=`` PROCEDURE `mostrar_citas_consulta` (IN `fecha` VARCHAR(20), IN `user_name` VARCHAR(60))  BEGIN
...
CREATE DEFINER=`` PROCEDURE `mostrar_lista_citas` (IN `nombresearch` VARCHAR(60), IN `filter` VARCHAR(1), IN `user_name` VARCHAR(60))  BEGIN
...
CREATE DEFINER=`` PROCEDURE `mostrar_perfil` (IN `user_name` VARCHAR(60))  BEGIN

	select * from v_perfil vp where vp.username = user_name; 

END$$

CREATE DEFINER=`` PROCEDURE `showProfile` (IN `name` VARCHAR(30) CHARSET utf8)  SELECT 
	us.Id_Usuario, 
    doc.Id_Doctor, 
    doc.Nombres, 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`` SQL SECURITY DEFINER VIEW `v_departamento`  AS  select `de`.`Id_Pais` AS `idPais`,`de`.`Id_Departamento` AS `idDepartamento`,`de`.`Descripcion` AS `nombre_departamento` from `departamento` `de` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`` SQL SECURITY DEFINER VIEW `v_distrito`  AS  select `di`.`Id_Provincia` AS `idProvincia`,`di`.`Id_Distrito` AS `idDistrito`,`di`.`Descripcion` AS `nombre_distrito` from `distrito` `di` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`` SQL SECURITY DEFINER VIEW `v_lista_doctor`  AS  select `doc`.`Id_Doctor` AS `id`,concat(`doc`.`Nombres`,' ',`doc`.`Apellido_Paterno`,' ',`doc`.`Apellido_Materno`) AS `nombre` from (`doctor` `doc` join `usuario` `usu` on(`usu`.`Id_Doctor` = `doc`.`Id_Doctor`)) where `usu`.`Activo` = 1 ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`` SQL SECURITY DEFINER VIEW `v_provincia`  AS  select `pr`.`Id_Departamento` AS `idDepartamento`,`pr`.`Id_Provincia` AS `idProvincia`,`pr`.`Descripcion` AS `nombre_pro` from `provincia` `pr` ;
...
--
ALTER TABLE `detalle_cuestionario_paciente`
  MODIFY `Id_Cuestionario_Paciente` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=18;
