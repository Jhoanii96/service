...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 01-09-2020 a las 04:44:23
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_lista_archivo`  AS  select `hc`.`Id_historia_clinica` AS `id_historial`,`ar`.`Id_Archivo` AS `id_archivo`,`ar`.`Subnombre` AS `nombre_archivo`,`ar`.`Enlace` AS `url`,`ar`.`Borrado` AS `isdelete`,`ar`.`Size` AS `size`,`ar`.`FechaCreado` AS `fecha_creado`,`ta`.`Nombre` AS `tipo_nom`,`us`.`Nombre` AS `username` from (((`historia_clinica` `hc` join `usuario` `us` on(`us`.`Id_Usuario` = `hc`.`Id_Usuario`)) join `archivo` `ar` on(`ar`.`Id_Historia_Clinica` = `hc`.`Id_historia_clinica`)) join `tipo_archivo` `ta` on(`ta`.`Id_Tipo_Archivo` = `ar`.`Id_Tipo_Archivo`)) ;
